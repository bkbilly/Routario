...
async function sendAiCopilotMessage() {
    ...
    try {
        ...
        const formattedText = renderMarkdown(data.response || 'No response returned.');
...
function buildHtmlTable(rows) {
    ...
    let ths = header.map(h => `<th style="padding:0.55rem 0.75rem;border:1px solid var(--border-color);background:var(--bg-secondary);text-align:left;font-weight:600;font-size:0.82rem;">${escapeHtml(h)}</th>`).join('');
    let trs = body.map(row => {
        let tds = row.map(c => `<td style="padding:0.45rem 0.75rem;border:1px solid var(--border-color);font-size:0.84rem;">${escapeHtml(c)}</td>`).join('');
        return `<tr>${tds}</tr>`;
    }).join('');
    ...
    let key = `__HTML_TABLE_${_tableCache.length}__`;
    _tableCache.push(tableHtml);
    return key;
}
...
function renderMarkdown(text) {
    ...
    // Headers
    html = html.replace(/^### (.*$)/gim, '<h3 style="font-size:1.02rem;color:var(--accent-primary);margin-top:1rem;margin-bottom:0.4rem;">$1</h3>');
    html = html.replace(/^## (.*$)/gim, '<h2 style="font-size:1.1rem;color:var(--text-primary);margin-top:1.2rem;margin-bottom:0.5rem;border-bottom:1px solid var(--border-color);padding-bottom:0.3rem;">$1</h2>');
    html = html.replace(/^# (.*$)/gim, '<h1 style="font-size:1.2rem;color:var(--text-primary);margin-bottom:0.6rem;">$1</h1>');
    ...
    // Restore pre-rendered table placeholders
    html = html.replace(/__HTML_TABLE_(\d+)__/g, (match, idx) => {
        return _tableCache[idx] || '';
    });
